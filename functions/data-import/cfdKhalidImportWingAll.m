function [table_3d,cell_of_tables] = cfdKhalidImportWingAll(folder_name)
...
for i = 1:num_files
    if contains(dat_files(i).name,'.dat') && contains(dat_files(i).name,'i=')
        [table_,time] = cfdKhalidImportWingTime(dat_files(i).name);
...
% sort spanwise coordinate
for i = 1:size(table_3d.Y,1)
    [~,idx_sort] = sort( table_3d.Y(1,:) );
    for j = 1:length(var_names)
        table_3d.(var_names{j}) = table_3d.(var_names{j})(:,idx_sort);
    end
end
